...
// PG 密码环境变量（edgelink-pg-config 节点从 process.env.PG_PASSWORD 读取）
// 🔧 不再明文兜底，统一从用户环境变量 EDGE_PG_PASSWORD 读取
process.env.PG_PASSWORD = process.env.PG_PASSWORD || process.env.EDGE_PG_PASSWORD || '';
 ...
 ******************************************************************************/
    ...
    /* 必须绝对路径：相对路径会随启动时的当前目录解析（曾因此加载到错误的流文件） */
    flowFile: 'D:/nodered/data/flows.json',
 ...
 ******************************************************************************/
     ...
     */
    httpStatic: 'D:/nodered/data/static',
    /**
 ...
 ******************************************************************************/
    ...
    /**
     * 全局 Function 节点超时（秒）— 防止单个 function node 阻塞 event loop。
     * 超时后 Node-RED 会强制终止该节点并记录警告，不影响其他节点运行。
     * 设为 0 表示无超时（仅调试用，生产环境务必设置）。
     */
    globalFunctionTimeout: 15,
   ...
   functionGlobalContext: {
    ...
    mcSim_debug: true,
    edge_debug: true,
    mcSim_simulationMode: false,
    mcSim_backendHost: '127.0.0.1',
    mcSim_backendPort: 9099,
    mcSim_backendUser: 'admin',
    mcSim_backendPass: process.env.EDGE_BACKEND_PASS || '',
    mcSim_apiKey: process.env.EDGE_API_KEY || '',
    // 🔧 凭据已迁移到用户环境变量（setx EDGE_*），不再明文存储；缺失时启动日志会有告警
    edge_pgPassword: process.env.EDGE_PG_PASSWORD || '',
    // ===== EdgeLink 后端凭据（edge_ 前缀，Day 6 后使用）=====
    edge_backendHost: '127.0.0.1',
    edge_backendPort: 9099,
    edge_backendUser: process.env.EDGE_BACKEND_USER || 'admin',
    edge_backendPass: process.env.EDGE_BACKEND_PASS || '',
    edge_apiKey: process.env.EDGE_API_KEY || '',
    // ===== EdgeLink 多实例标识（每台 Node-RED 实例必须唯一）=====
    // v2.0 自动注册后，以下字段将由系统自动管理，无需手动配置
    // 当前用于 v1.0 手动配置验证
    edge_nodeId: 1001,
    edge_hostPcIp: '192.168.0.179:1880',
    // ===== Bootstrap 配置（v1.0 手动配置，v2.0 将自动注册）=====
    edge_bootstrapKey: 'pc-001',
    edge_bootstrapSecret: process.env.EDGE_BOOTSTRAP_SECRET || '',
    // ===== v2.0 自动注册预留（实施后启用）=====
    // edge_backendHost: '127.0.0.1',
    // edge_backendPort: 9099
},
...
// ===== EdgeLink 凭据缺失自检（启动时打印，便于发现环境变量未配置）=====
void (function () {
    ['EDGE_PG_PASSWORD', 'EDGE_BACKEND_PASS', 'EDGE_API_KEY', 'EDGE_BOOTSTRAP_SECRET'].forEach(function (k) {
        if (!process.env[k]) console.warn('[settings] ⚠ 环境变量 ' + k + ' 未配置，相关功能将不可用（用 setx 配置后重启 Node-RED）');
    });
})();
