...
> 日期：2026-07-22  
> 状态：已评审，待实施  
> 改进：两阶段 Bootstrap + 采集节点专用角色 + 机器指纹处理 IP 变更
...
| 改进 | 效果 |
|---|---|
| **两阶段 Bootstrap** | `/auto` 只发密钥不发凭据，攻击者拿到 `secret_key` 也无法直接获取业务配置 |
| **`edge_collector` 专用角色** | 节点被攻陷 ≠ 后端沦陷，影响范围可控 |
| **机器指纹处理 IP 变更** | `hostname + MAC` 稳定标识，IP 变更自动映射，无僵尸记录 |
...
plc:driver:list     -- 获取驱动 schema（用于动态表单）
...
Node-RED 启动
    ...
    │       └─ 返回: { nodeKey, nodeName, hostPcIp, secretKey, isNew }
    │
...
Node-RED 启动（IP 从 10.81.1.101 变成 10.81.1.102）
    ...
    ├─ 读取本地 edge_secret.json → hostPcIp 不匹配 → 但 fingerprint 相同
    │
...
Node-RED 启动
    │
    ├─ 检测到 hostPcIp = "10.81.1.102:1880"
    ├─ 读取本地 edge_secret.json → hostPcIp 匹配、secretKey 存在
    │
...
**重要：`backendUser`/`backendPass` 是 `edge_collector` 角色的账号，不是 admin。密码由系统自动生成，存储在 `sys_config` 表中，所有节点共用，定期轮换。**
...
### 5.3 `edge_collector` 账号密码管理

```python
# bootstrap_controller.py: 首次注册时检查 edge_collector 账号是否存在
# 不存在则自动创建一个随机密码的账号，密码存储在 sys_config 表中
async def _ensure_collector_account(db):
    ...
    if not existing:
        ...
        # 存储密码到 sys_config，供所有节点共用
        set_config(db, 'edge_collector_password', password)
    return get_config(db, 'edge_collector_password')
```
...
2. 创建 `edge_collector` 角色和权限
3. 创建 `edge_collector` 系统账号，密码存储到 `sys_config`

### Day 10（集成测试与回归）

1. 后端 `/plc/config/bootstrap/auto` 接口开发
2. 后端 `/plc/config/bootstrap` 接口改造（增加 secret_key 验证）
3. Node-RED `sf-config-manager` 两阶段调用逻辑
4. Node-RED 机器指纹计算 + 本地缓存
5. 端到端测试：新节点自动注册、IP 变更自动映射、配置更新下发

---

## 九、风险与限制

1. **机器指纹稳定性**：更换网卡/主板/重装系统会导致指纹变更，需要管理员手动关联旧记录
2. **统一账号风险**：`edge_collector` 统一账号，一个节点泄露影响所有，建议定期轮换密码
3. **内网依赖**：`/auto` 接口无鉴权，依赖内网隔离，不适用于公网环境
4. **实施复杂度**：涉及数据库变更、角色创建、Node-RED 大改，需要充分测试

---

## 十、评审确认

- [x] 两阶段 Bootstrap 设计合理
- [x] `edge_collector` 专用角色权限最小化
- [x] 机器指纹处理 IP 变更可行
- [x] 统一 `edge_collector` 账号密码管理可接受
- [x] 实施计划纳入 Day 9/10 合理
