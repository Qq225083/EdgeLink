...
module.exports = function (RED) {
  ...
  function MitsubishiReadNode(config) {
    ...
    node.on('input', function (msg) {
      // 动态IP覆盖: msg.payload.plcIp/plcPort 可覆盖 config 的固定值, 一个节点采多台PLC
      var cfg = node.plcConfig;
      var plc = {
        name: cfg.name, host: msg.payload.plcIp || cfg.host,
        port: msg.payload.plcPort || cfg.port, frame: cfg.frame,
        networkNo: cfg.networkNo, stationNo: cfg.stationNo,
      ...
      if (global._mcLocks[lockKey] && (Date.now() - global._mcLocks[lockKey] < 60000)) {
        // 🔧 正在读取中，丢弃本次请求（下一轮 inject 自然会重试）
        // 不再使用 setTimeout(self-send) 防止消息克隆风暴导致 OOM
        node.status({ fill: 'yellow', shape: 'dot', text: 'busy, skip' });
        return;
      }
      ...
      function processGroup(gi) {
        if (gi >= groups.length) {
          ...
          msg.payload = {
            success: !hasFailed,
            deviceId: msg.payload.id || msg.payload.deviceName || plc.name || (plc.host + ':' + plc.port),
            data: output,
