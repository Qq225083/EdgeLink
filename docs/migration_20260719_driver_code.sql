-- EdgeLink 动态驱动架构 Day 1 数据库迁移脚本
-- 适用场景：已有数据库实例升级到支持 driver_code / plc_driver 的版本
-- 说明：
--   1. MySQL 8.0.16+ 支持 ALTER TABLE ... ADD COLUMN IF NOT EXISTS；低于该版本请使用 migration_20260719_driver_code_mysql57.sql。
...
-- 2. 给设备表增加 driver_code
ALTER TABLE plc_device ADD COLUMN IF NOT EXISTS driver_code VARCHAR(50) NULL COMMENT '驱动编码（对应 plc_driver.driver_code）';

-- 3. 给协议兼容表增加 driver_code（默认 UNKNOWN，避免错误 fallback）
ALTER TABLE plc_protocol_compat ADD COLUMN IF NOT EXISTS driver_code VARCHAR(50) NOT NULL DEFAULT 'UNKNOWN' COMMENT '驱动编码（对应 plc_driver.driver_code）';
...
-- 西门子/欧姆龙 RS232C 的真实协议不是 Modbus，当前没有对应驱动时，宁可标记为 UNKNOWN
...
-- 8. 索引：加速按品牌/系列/通信方式查找驱动
CREATE INDEX IF NOT EXISTS idx_plc_protocol_compat_brand_series_com ON plc_protocol_compat (plc_brand, plc_series, com_type);
